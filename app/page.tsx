...
export default function Review() {
  ...
  const requestBody = {
    ...
    "password": passwordInputVal
      ...
      if (!res.ok) {
        ...
        userContext.setUsername(usernameInputVal);
        userContext.setUserId(authDTO['userId']);
        userContext.setAuthToken(authDTO['token'])
        success = true
      }
    } catch (err: any) {
 ...
 try {
  ...
  <button type="submit" className="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 m-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700 cursor-pointer">Register</button>
      <span className="mt-2 ml-2">Already have an account? <u onClick={() => {setIsLoginMode(true)}}>Log In</u></span>
      </form>
